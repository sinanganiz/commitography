// The metric family contract (ADR-0024 clause 1): what every family declares
...
// InputKind is one of the inputs a family may require (ADR-0024 clause 1).
type InputKind string

// The four input kinds. There is no fifth: ADR-0075 keeps the set at four.
const (
	InputCommitRecords   InputKind = "commit-records"
	InputReplayState     InputKind = "replay-state"
	InputWorktree        InputKind = "worktree"
	InputExternalService InputKind = "external-service"
)

// InputKinds returns the four input kinds, in the order ADR-0024 clause 1
// lists them.
func InputKinds() []InputKind {
	return []InputKind{InputCommitRecords, InputReplayState, InputWorktree, InputExternalService}
}
...
type FamilyDeclaration struct {
	// Name is the family's name in ADR-0024 clause 5.
	Name string
	// Inputs are the inputs the family requires, from the four kinds.
	Inputs []InputKind
	// Namespace is the report namespace the family owns, as docs/metrics.md
	// gives it (ADR-0024 clause 4, ADR-0062 clause 3).
	Namespace string
