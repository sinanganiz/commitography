...
| `docs/metrics.md` | **Binding.** The authoritative metric catalogue (ADR-0062). |
| `docs/conventions.md` | **Guidance.** A justified deviation is acceptable. |

A rule never lives in two of these. Do not treat guidance as binding, and do not
...
Status lives only in `docs/work/INDEX.md` and is changed by the author. Package
...
Do not claim an item is satisfied without its evidence.
...
7. **Changing a metric's meaning requires a version increment** and a change to
   `docs/metrics.md` in the same commit. Including a threshold change or a
   default change. See ADR-0031, ADR-0062.
