...
- **A package that writes into a reserved section must own the schema.** Three
  packages in a row needed `docs/report-schema.json` and only two listed it.
  If a package changes what the report contains, it changes what validates it.
- **"Belongs to the analysis plane" is not "settable by the operator."**
  Cardinality limits are embedded, enter the cache key, and are fixed by the
  catalogue. Assign a value's plane by whether it changes a metric, and its
  settability separately.
- **A record that says "this needs its own decision" is a promise to make one.**
  ADR-0068 wrote that a matcher needing more than exact comparison was out of
...
- **A determinism guarantee must name every input that can move the output.**
  ...
  it calls read.
- **Confirm the data a transformation needs is produced.** WP-0013 was written
  to track line ownership while the only records available carried per-file
  counts. Before writing a package that transforms data, find the package that
  produces that data.
- **A linear model of a branched thing is a decision, not a simplification.**
...
- **When a package hits a collision, look for the defect that predates it.**
  ...
  would have preserved the original error.
...
| [0013](0013-replay-stage.md) | pipeline | Replay stage and ownership map | ADR-0020, ADR-0051, ADR-0072, ADR-0073, ADR-0033, ADR-0019, ADR-0052 | WP-0012 | Ready |
